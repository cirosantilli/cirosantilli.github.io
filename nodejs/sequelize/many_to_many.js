...
// AnimalTag is the name of the relation table.
...
async function reset() {
  ...
  bat = await Animal.create({ name: 'bat' })
  frog = await Animal.create({ name: 'frog' })
  dog = await Animal.create({ name: 'dog' })
  flying = await Tag.create({ body: 'flying' })
  mammal = await Tag.create({ body: 'mammal' })
  reptile = await Tag.create({ body: 'reptile' })
...
const batTags = await bat.getTags({order: [['body', 'ASC']]})
assert.strictEqual(batTags[0].body, 'flying');
assert.strictEqual(batTags[1].body, 'mammal');
assert.strictEqual(batTags.length, 2);
assert.strictEqual(await bat.countTags(), 2)

const frogTags = await frog.getTags({order: [['body', 'ASC']]})
assert.strictEqual(frogTags.length, 0);

const dogTags = await dog.getTags({order: [['body', 'ASC']]})
assert.strictEqual(dogTags[0].body, 'mammal');
...
const batTagsNoJoin = await bat.getTags({order: [['body', 'ASC']], joinTableAttributes: []})
assert.strictEqual(batTagsNoJoin[0].body, 'flying');
assert.strictEqual(batTagsNoJoin[1].body, 'mammal');
...
// Same as get but with the animal ID instead of the model object.
{
  ...
  assert.strictEqual(batTags[0].body, 'flying');
  assert.strictEqual(batTags[0].Animals[0].name, 'bat');
  ...
  assert.strictEqual(batTags[1].Animals[0].name, 'bat');
...
console.error(rows);
assert.strictEqual(rows[0].body, 'flying');
assert.strictEqual(rows[0].Animals[0].name, 'bat');
assert.strictEqual(rows[0].Animals.length, 1);
assert.strictEqual(rows[1].body, 'mammal');
assert.strictEqual(rows[1].Animals[0].name, 'bat');
assert.strictEqual(rows[1].Animals[1].name, 'dog');
...
if (false) {
  ...
  assert.strictEqual(rows[2].body, 'reptile');
...
const flyingTagged = await flying.getAnimals({order: [['name', 'ASC']]})
assert.strictEqual(flyingTagged[0].name, 'bat');
assert.strictEqual(flyingTagged.length, 1);

const mammalTagged = await mammal.getAnimals({order: [['name', 'ASC']]})
assert.strictEqual(mammalTagged[0].name, 'bat');
assert.strictEqual(mammalTagged[1].name, 'dog');
assert.strictEqual(mammalTagged.length, 2);

const reptileTagged = await reptile.getAnimals({order: [['name', 'ASC']]})
...
assert.strictEqual(rows[0].name, 'bat')
assert.strictEqual(parseInt(rows[0].count, 10), 2)
assert.strictEqual(rows[1].name, 'dog')
...
assert.strictEqual(rows[1].name, 'dog')
assert.strictEqual(parseInt(rows[1].count, 10), 1)
assert.strictEqual(rows[2].name, 'frog')
...
assert.strictEqual(rows[1].name, 'frog')
...
const tag3 = await bat.createTag({'body': 'tag3'})
