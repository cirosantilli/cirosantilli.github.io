...
Paging translates linear addresses,
what is left after segmentation translated logical addresses,
into physical addresses, what actually goes go to RAM wires:

    (logical) ------------------> (linear) ------------> (physical)
                 segmentation                 paging

Paging is only available on protected mode. The use of paging protected mode is optional.
Paging is on iff the `PG` bit of `cr0` is set.
...
and largely replaced it for the implementation of virtual memory in modern OSs such as Linux
...
This means that it faster and simpler to allocate and deallocate pages for that purpose.
...
-   PAE and no PSE: `2 | 9 | 9 | 12`
    ...
    `cr3` now points to PDPTs which must be on the fist four 4GB of memory
    and aligned on 32 bit multiples for addressing efficiency.
...
-   PAE and PSE: `2 | 9 | 21`
    ...
    This leaves 23 bits. Leaving 2 for the PDPT to keep things uniform
    with the PAE case without PSE leaves 21 for offset,
