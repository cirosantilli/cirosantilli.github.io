...
This approach is called https://en.wikipedia.org/wiki/Backward_design[backward design].
...
<div class="canvas-demo">
    <div>enable: <input type="checkbox" id="canvas-enable"></input> &lt;-- (click this to run!!!)</div>
    ...
    <div>save images: <input type="checkbox" id="canvas-save-images-checkbox"></input> (some images are skipped if FPS is high)</div>
    <div>FPS limit: <input type="number" min="10" id="canvas-fps-limit" value=""></input></div>
    <div>FPS granule: <input type="number" min="10" id="canvas-fps-granule" value="5"></input></div>
    <div>total frames: <span id="canvas-total-frames"></span></div>
    <div>FPS: <span id="canvas-fps"></span></div>
    <canvas id="canvas-canvas" style="border:1px solid black;"></canvas>
</div>
<script>
(function() {
    ...
    function main() {
        ...
        var demo_id = 'canvas';
        var canvas = document.getElementById(demo_id + '-canvas');
        var ctx = canvas.getContext('2d');
        var pixel_size = 1;
        var t = 0;
        var fps_span = document.getElementById(demo_id + '-fps');
        var fps_last_date = new Date();
        var fps_granule_input = document.getElementById(demo_id + '-fps-granule');
        var canvas_width_input = document.getElementById(demo_id + '-width-input');
        // https://stackoverflow.com/questions/19764018/controlling-fps-with-requestanimationframe
        var save_images_checkbox = document.getElementById(demo_id + '-save-images-checkbox');
        var total_frames_span = document.getElementById(demo_id + '-total-frames');
        total_frames_span.innerHTML = t.toString();

        /* FPS limit. */
        var fps_limit_input = document.getElementById(demo_id + '-fps-limit');
        var fps_limit_last_render = Date.now();
        var now, fps_limit_then, fps_limit_elapsed;
        fps_limit_then = Date.now();

        /* Enable disable. */
        var enable_checkbox = document.getElementById(demo_id + '-enable');
