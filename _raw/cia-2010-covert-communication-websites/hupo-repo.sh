...
ls -1 "$outdir" | while IFS= read -r y; do
  if [ $y -ge "$from" ] && [ $y -le "$to" ]; then
    cd "$outdir/$y"
    printf "# Expired Domain Names by Day ${y}\n\nMethodology described at: https://cirosantilli.com/cia-2010-covert-communication-websites/expired-domain-trackers" > README.md
    git init
    git add .
